...
import 'package:wm_com_ivanna/src/pages/livraison/components/consommation/pro_format_livraison.dart'; 
...
class DetailConsommationLivraison extends StatefulWidget {
  const DetailConsommationLivraison({super.key, required this.tableRestaurantModel});
  final TableRestaurantModel tableRestaurantModel;

  @override
  State<DetailConsommationLivraison> createState() => _DetailConsommationLivraisonState();
}

class _DetailConsommationLivraisonState extends State<DetailConsommationLivraison> {
  final TableLivraisonController controller = Get.find();
  final LivraisonController livraisonController = Get.find();
  final MonnaieStorage monnaieStorage = Get.put(MonnaieStorage());
  final FactureLivraisonPDFA6 factureCartPDFA6 = Get.put(FactureLivraisonPDFA6());
  ...
  Widget build(BuildContext context) {
    ...
    var restaurants = livraisonController.restaurantList
        .where((p0) =>
                                                  ...
                                                  onPressed: () async {
                                                    refresh().then((value) =>
                                                        Navigator.pushNamed(
                                                            context,
                                                            LivraisonRoutes
                                                                .tableConsommationLivraisonDetail,
                                                            arguments: value));
                                                  },
                                                  icon: const Icon(
                                                      Icons.refresh,
  ...
  Widget totalCart() {
    ...
    return livraisonController.obx(
        onLoading: loadingPage(context),
        onEmpty: const Text('Aucune donnée'),
        onError: (error) => loadingError(context, error!), (state) {
      double sumCart = 0;
    ...
    return SpeedDial(
      ...
      speedDialChildren: <SpeedDialChild>[
        SpeedDialChild(
            ...
            label: 'Facture Pro',
            onPressed: () {
              ProFormatLivraisonPDFA6().generatePdf(widget.tableRestaurantModel.table,
          ...
          return StatefulBuilder(builder: (context, StateSetter setState) {
            return AlertDialog(
              ...
              content: SizedBox(
                  height: 200,
                  width: 200,
                  child: Obx(() => livraisonController.isFactureLoading
                      ? loading()
                      : Form(
                          key: livraisonController.factureFormKey,
                          child: Column(
                            children: [
                              nomClientWidget(),
                              telephoneWidget(),
              ...
              actions: <Widget>[
                TextButton(
                  onPressed: () {
                    final form =
                        livraisonController.factureFormKey.currentState!;
                    if (form.validate()) {
                      livraisonController.submitFacture(restaurants);
                      form.reset();
                      Navigator.pop(context, 'ok');
                    }
                  },
                ...
                TextButton(
                  onPressed: () {
                    ...
                    if (form.validate()) {
                      livraisonController.submitFacture(restaurants);
                      livraisonController.createFacturePDF(restaurants);
                      form.reset();
                      Navigator.pop(context, 'ok');
                    }
                  },
          ...
          return StatefulBuilder(builder: (context, StateSetter setState) {
            return AlertDialog(
              ...
              content: SizedBox(
                  height: 400,
                  width: 300,
                  child: Obx(() => livraisonController.isCreanceLoading
                      ? loading()
                      : Form(
                          key: livraisonController.creanceFormKey,
                          child: Column(
                            children: [
              ...
              actions: <Widget>[
                TextButton(
                  onPressed: () {
                    final form =
                        livraisonController.creanceFormKey.currentState!;
                    if (form.validate()) {
                      livraisonController.submitFactureCreance(restaurants);
                      form.reset();
                      Navigator.pop(context, 'ok');
                    }
                  },
                ...
                TextButton(
                  onPressed: () {
                    ...
                    if (form.validate()) {
                      livraisonController.submitFactureCreance(restaurants);
                      livraisonController.createPDFCreance(restaurants);
                      form.reset();
                      Navigator.pop(context, 'ok');
                    }
                  },
  ...
  Widget nomClientWidget() {
    return Container(
        ...
        child: TextFormField(
          controller: livraisonController.nomClientController,
  ...
  Widget telephoneWidget() {
    return Container(
        margin: const EdgeInsets.only(bottom: p20),
        child: TextFormField(
          controller: livraisonController.telephoneController,
          decoration: InputDecoration(
            border:
                OutlineInputBorder(borderRadius: BorderRadius.circular(10.0)),
            labelText: 'Téléphone',
  ...
  Widget nomClientACreditWidget() {
    return Container(
        margin: const EdgeInsets.only(bottom: p20),
        child: TextFormField(
          controller: livraisonController.nomClientAcreditController,
          decoration: InputDecoration(
            border:
                OutlineInputBorder(borderRadius: BorderRadius.circular(10.0)),
            labelText: 'Nom du client',
  ...
  Widget telephoneACreditWidget() {
    return Container(
        margin: const EdgeInsets.only(bottom: p20),
        child: TextFormField(
          controller: livraisonController.telephoneAcreditController,
          decoration: InputDecoration(
            border:
                OutlineInputBorder(borderRadius: BorderRadius.circular(10.0)),
            labelText: 'Téléphone',
  ...
  Widget adresseACreditWidget() {
    return Container(
        margin: const EdgeInsets.only(bottom: p20),
        child: TextFormField(
          controller: livraisonController.addresseAcreditController,
          decoration: InputDecoration(
            border:
                OutlineInputBorder(borderRadius: BorderRadius.circular(10.0)),
            labelText: 'Adresse',
    ...
    return Container(
        ...
        child: TextButton.icon(
            onPressed: () async {
              ...
              livraisonController.delaiPaiementAcredit = dateTime;
            },
