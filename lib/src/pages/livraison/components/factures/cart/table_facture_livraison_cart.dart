...
class TableFactureLivraisonCart extends StatefulWidget {
  const TableFactureLivraisonCart({Key? key, required this.factureList})
      : super(key: key);
  final List<RestaurantModel> factureList;

  @override
  State<TableFactureLivraisonCart> createState() => _TableFactureLivraisonCartState();
}

class _TableFactureLivraisonCartState extends State<TableFactureLivraisonCart> {
        ...
        columns: [
          ...
          DaviColumn(
            ...
            stringValue: (row) {
              double total = 0;
              total +=
                  double.parse(row.qty) * double.parse(row.price);
