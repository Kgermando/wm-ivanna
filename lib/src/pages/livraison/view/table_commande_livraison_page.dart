...
class TableCommandeLivraisonPage extends StatefulWidget {
  const TableCommandeLivraisonPage({super.key});

  @override
  State<TableCommandeLivraisonPage> createState() => _TableCommandeLivraisonPageState();
}

class _TableCommandeLivraisonPageState extends State<TableCommandeLivraisonPage> {
  final TableLivraisonController controller =
      Get.put(TableLivraisonController());
  final LivraisonController restaurantController =
      Get.put(LivraisonController());
  final GlobalKey<ScaffoldState> scaffoldKey = GlobalKey();
  String title = "Livraison";
  String subTitle = "Commandes";
    ...
    return Scaffold(
        ...
        floatingActionButton: Responsive.isMobile(context)
            ? FloatingActionButton(
                tooltip: "Ajouter automobile",
                child: const Icon(Icons.add),
                onPressed: () => tableDialog())
            : FloatingActionButton.extended(
                label: const Text("Ajouter automobile"),
                tooltip: "Ajouter automobile",
  ...
  Widget tableWidget(TableRestaurantModel data) {
    return restaurantController.obx(
      ...
      var restaurant =
          state!.where((element) => element.table == data.table && element.statutCommande == 'false').toList();
      Color? color;
      bool isBusy = false;
      ...
      return InkWell(
        onTap: () => Get.toNamed(LivraisonRoutes.tableCommandeLivraisonDetail,
            arguments: data),
        child: ClipRRect(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Container(),
                Icon((isBusy) ? Icons.delivery_dining : Icons.delivery_dining_outlined,
                    size: 60, color: color),
