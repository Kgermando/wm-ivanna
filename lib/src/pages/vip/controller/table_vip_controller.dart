...
import 'package:wm_com_ivanna/src/global/store/vip/table_vip_store.dart'; 
import 'package:wm_com_ivanna/src/models/restaurant/table_restaurant_model.dart';
import 'package:wm_com_ivanna/src/pages/auth/controller/profil_controller.dart';
import 'package:wm_com_ivanna/src/utils/info_system.dart';

class TableVipController extends GetxController
    with StateMixin<List<TableRestaurantModel>> {
  TableVipStore tableVipStore = TableVipStore();
  ...
  void getList() async {
    tableVipStore.getAllData().then((response) {
  ...
  detailView(int id) => tableVipStore.getOneData(id);

  void deleteData(int id) async {
    try {
      _isLoading.value = true;
      await tableVipStore.deleteData(id).then((value) {
        tableRestaurantList.clear();
        getList();
  ...
  Future submitCommande() async {
    try {
      ...
      final dataItem = TableRestaurantModel(
          ...
          sync: "new",
          async: "async");
      await tableVipStore.insertData(dataItem).then((value) async {
        clear();
        getList();
        // Get.back();
  ...
  Future submitUpdate(TableRestaurantModel data) async {
    try {
      ...
      final dataItem = TableRestaurantModel(
          ...
          sync: "update",
          async: "async");
      await tableVipStore.updateData(dataItem).then((value) {
        getList();
        Get.back();
        Get.snackbar(
