...
class RestaurantController extends GetxController
  ...
  final CreanceRestaurantPDFA6 creanceRestaurantPDFA6 =
      Get.put(CreanceRestaurantPDFA6());
  ...
  void getList() async {
    restaurantStore.getAllData().then((response) {
  ...
  detailView(int id) => restaurantStore.getOneData(id);
  ...
  void deleteData(int id) async {
    try {
      ...
      await restaurantStore.deleteData(id).then((value) {
        restaurantList.clear();
        getList();
  ...
  Future addCommande(ProductModel productModel,
    ...
    try {
      ...
      await restaurantStore.insertData(dataItem).then((value) async {
        getList();
        // Get.back();
  ...
  Future statutSubmit(RestaurantModel restaurantModel, String statut) async {
    try {
      ...
      await restaurantStore.updateData(dataItem).then((value) {
        getList();
        // Get.back();
        Get.snackbar(
  ...
  Future<void> createFacturePDF(List<RestaurantModel> restaurants) async {
    try {
      ...
      factureRestaurantPDFA6.generatePdf(facture!, monnaieStorage.monney);
      clear();
      _isFactureLoading.value = false;
    } catch (e) {
      _isFactureLoading.value = false;
  ...
  Future<void> submitFactureCreance(List<RestaurantModel> restaurants) async {
    try {
      ...
      await creanceRestaurantStore.insertData(creanceCartModel).then((value) {
  ...
  Future<void> createPDFCreance(List<RestaurantModel> restaurants) async {
    try {
      ...
      creanceRestaurantPDFA6.generatePdf(creance!, monnaieStorage.monney);
  ...
  Future<void> venteHisotory(List<RestaurantModel> restaurants) async {
    restaurants.forEach((item) async {
      ...
      await venteEffectueRestStore
          .insertData(venteCartModel)
          .then((value) async {
        await restaurantStore.deleteData(item.id!);
      });
    });
  }
}
