...
class ProduitModelVipApi extends GetConnect {
  var client = http.Client();

  Future<List<ProductModel>> getAllData() async {
    Map<String, String> header = headers;

    var resp = await client.get(prodModelVipUrl, headers: header);
  ...
  Future<ProductModel> getOneData(int id) async {
    ...
    var getUrl = Uri.parse("$mainUrl/prod-mode-vips/$id");
  ...
  Future<ProductModel> insertData(ProductModel productModel) async {
    ...
    var resp =
        await client.post(addProdModelVipUrl, headers: header, body: body);
    if (resp.statusCode == 200) {
      return ProductModel.fromJson(json.decode(resp.body));
  ...
  Future<ProductModel> updateData(ProductModel productModel) async {
    ...
    var data = productModel.toJson();
    var body = jsonEncode(data);
    var updateUrl = Uri.parse("$mainUrl/prod-mode-vips/update-produit-model/");
  ...
  Future<void> deleteData(int id) async {
    ...
    var deleteUrl =
        Uri.parse("$mainUrl/prod-mode-vips/delete-produit-model/$id");
