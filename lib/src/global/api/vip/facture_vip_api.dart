...
class FactureVipApi extends GetConnect {
  var client = http.Client();

  Future<List<FactureRestaurantModel>> getAllData() async {
    Map<String, String> header = headers;

    var resp = await client.get(factureVipUrl, headers: header);
  ...
  Future<FactureRestaurantModel> getOneData(int id) async {
    ...
    var getUrl = Uri.parse("$mainUrl/facture-vips/$id");
  ...
  Future<FactureRestaurantModel> insertData(
    ...
    var resp = await client.post(addFactureVipUrl, headers: header, body: body);
    if (resp.statusCode == 200) {
      return FactureRestaurantModel.fromJson(json.decode(resp.body));
  ...
  Future<FactureRestaurantModel> updateData(
    ...
    var data = dataItem.toJson();
    var body = jsonEncode(data);
    var updateUrl = Uri.parse("$mainUrl/facture-vips/update-facture/");
  ...
  Future<void> deleteData(int id) async {
    ...
    var deleteUrl = Uri.parse("$mainUrl/facture-vips/delete-facture/$id");
