...
class CreanceTerrassApi extends GetConnect {
  var client = http.Client();

  Future<List<CreanceRestaurantModel>> getAllData() async {
    Map<String, String> header = headers;

    var resp = await client.get(creanceTerrasseUrl, headers: header);
  ...
  Future<CreanceRestaurantModel> getOneData(int id) async {
    ...
    var getUrl = Uri.parse("$mainUrl/creance-terrasses/$id");
  ...
  Future<CreanceRestaurantModel> insertData(CreanceRestaurantModel dataItem) async {
    ...
    var resp =
        await client.post(addCreanceTerrasseUrl, headers: header, body: body);
    if (resp.statusCode == 200) {
      return CreanceRestaurantModel.fromJson(json.decode(resp.body));
  ...
  Future<CreanceRestaurantModel> updateData(CreanceRestaurantModel dataItem) async {
    ...
    var body = jsonEncode(data);
    var updateUrl =
        Uri.parse("$mainUrl/creance-terrasses/update-creance/");
  ...
  Future<void> deleteData(int id) async {
    ...
    var deleteUrl =
        Uri.parse("$mainUrl/creance-terrasses/delete-creance/$id");
