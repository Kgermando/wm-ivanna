...
class TableTerrasseApi extends GetConnect {
  var client = http.Client();

  Future<List<TableRestaurantModel>> getAllData() async {
    Map<String, String> header = headers;

    var resp = await client.get(tableTerrasseUrl, headers: header);
  ...
  Future<TableRestaurantModel> getOneData(int id) async {
    ...
    var getUrl = Uri.parse("$mainUrl/table-terrasses/$id");
  ...
  Future<TableRestaurantModel> insertData(
      TableRestaurantModel dataItem) async {
    Map<String, String> header = headers;
    ...
    var resp =
        await client.post(addTableTerrasseUrl, headers: header, body: body);
    if (resp.statusCode == 200) {
      return TableRestaurantModel.fromJson(json.decode(resp.body));
  ...
  Future<TableRestaurantModel> updateData(
      TableRestaurantModel dataItem) async {
    Map<String, String> header = headers;

    var data = dataItem.toJson(id: dataItem.id!);
    var body = jsonEncode(data);
    var updateUrl = Uri.parse("$mainUrl/table-terrasses/update-table/");
  ...
  Future<void> deleteData(int id) async {
    ...
    var deleteUrl = Uri.parse("$mainUrl/table-terrasses/delete-table/$id");
