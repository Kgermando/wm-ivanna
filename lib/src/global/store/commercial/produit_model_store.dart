...
class ProduitModelStore {
  static final ProduitModelStore _singleton = ProduitModelStore._internal();
  factory ProduitModelStore() {
    return _singleton;
  }
  ProduitModelStore._internal();

  static const String tableName = 'produit-models';
  ...
  Future<List<ProductModel>> getAllData() async {
    ...
    for (var snapshot in data) {
      final ProductModel dataItem =
          ProductModel.fromJson(snapshot.value);
  ...
  deleteData(int id) async {
    await store.delete(await _db, finder: Finder(filter: Filter.equals('id', id)));
  }
